...
export async function POST(request: NextRequest) {
  try {
    ...
    // const isValid = validateHmac(formData, process.env.HITPAY_WEBHOOK_SALT || "")
    ...
    if (!salt) {
      console.error("Missing HITPAY_WEBHOOK_SALT");
      return new Response("Webhook not configured", { status: 500 });
    }

    const isValid = validateHmac(formData, salt);
    ...
    if (orderStatus === "paid") {
      console.log(
        `Payment successful for order ${reference_number}, creating Detrack order`,
      );
      ...
      try {
        ...
        if (orderLookupError) {
          console.error(
            `Error looking up full order ID for ${reference_number}:`,
            orderLookupError,
          );
          ...
          console.log(
            `Found full order ID ${fullOrderId} for short ID ${reference_number}`,
          );

          // Update all parcels for this order to have the same status
          console.log(
            `Updating status for all parcels in order ${reference_number}`,
          );
          ...
          if (parcelUpdateError) {
            console.error(
              `Error updating parcel statuses for order ${reference_number}:`,
              parcelUpdateError,
            );
          } else {
            console.log(
              `Updated status for all parcels in order ${reference_number}`,
            );
          }
        }
      } catch (detrackError) {
